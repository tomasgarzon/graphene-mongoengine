...
from .utils import is_valid_mongoengine_document, get_document_fields
from .registry import Registry, get_global_registry


def construct_fields(options):
    _model_fields = get_document_fields(options.document)
    ...
    for name, field in _model_fields.items():
        ...
        if is_not_in_only or is_excluded or is_no_backref:
            ...
            continue
        converted = convert_mongoengine_field(field, options.registry)
        if not converted:
            continue
...
class MongoEngineObjectTypeMeta(ObjectTypeMeta):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        assert is_valid_mongoengine_document(options.document), (
            'You need to pass a valid Django Model in {}.Meta, received "{}".'
        ).format(name, options.document)
...
class MongoEngineObjectType(six.with_metaclass(MongoEngineObjectTypeMeta, ObjectType)):
    ...
    def is_type_of(cls, root, context, info):
        ...
        if isinstance(root, cls):
            return True
        if not is_valid_mongoengine_document(type(root)):
            raise Exception((
                'Received incompatible instance "{}".'
            ).format(root))
        model = root.__class__
        return model == cls._meta.document

    @classmethod
    def get_node(cls, id, context, info):
        try:
            return cls._meta.document.objects.get(pk=id)
